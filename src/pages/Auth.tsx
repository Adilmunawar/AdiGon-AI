...
import { Sparkles, Zap, Code, Brain, Shield, Star } from "lucide-react";
...
const features = [
  ...
  { icon: Brain, title: "Deep Research", desc: "Analysis on any topic" },
  { icon: Zap, title: "Real-time Streaming", desc: "Instant responses" },
  { icon: Shield, title: "Secure & Private", desc: "Data stays protected" },
];
  ...
  return (
    <div className="h-screen flex bg-background overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[50%] relative overflow-hidden">
        ...
        <div className="absolute top-[10%] left-[5%] w-64 h-64 rounded-full bg-primary/[0.06] blur-[80px] animate-pulse" />
        <div className="absolute bottom-[15%] right-[10%] w-72 h-72 rounded-full bg-primary/[0.05] blur-[100px] animate-pulse" style={{ animationDelay: '2s' }} />
        ...
        <div className="relative z-10 flex flex-col justify-between p-8 xl:p-10 w-full h-full">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <span className="text-base font-bold tracking-tight text-foreground">AdiGon AI</span>
            </div>

            <div className="grid grid-cols-4 gap-2 mb-6">
              {stats.map(({ value, label }) => (
                <div key={label} className="text-center py-2 px-1 rounded-lg bg-background/40 border border-border/40 backdrop-blur-sm">
                  <div className="text-sm font-bold text-foreground">{value}</div>
                  <div className="text-[9px] text-muted-foreground font-medium uppercase tracking-wider mt-0.5">{label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="max-w-md space-y-4 flex-shrink-0">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/8 border border-primary/12">
              <Sparkles className="w-3 h-3 text-primary" />
              <span className="text-[10px] font-semibold text-primary uppercase tracking-wider">Powered by Gemini 2.5</span>
            </div>

            <h1 className="text-3xl xl:text-4xl font-bold tracking-tight text-foreground leading-[1.1]">
              Think bigger.
              <br />
            ...
            <div className="grid grid-cols-2 gap-2">
              {features.map(({ icon: Icon, title, desc }) => (
                <div key={title} className="p-2.5 rounded-lg bg-background/60 border border-border/50 backdrop-blur-sm hover:border-primary/20 transition-all duration-300 group">
                  <Icon className="w-3.5 h-3.5 text-primary mb-1.5 group-hover:scale-110 transition-transform" />
          ...
          <div className="space-y-3">
            ...
            <div className="relative p-3.5 rounded-xl bg-background/50 border border-border/50 backdrop-blur-sm overflow-hidden">
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] text-foreground/90 leading-relaxed mb-1.5 italic">
                    "{testimonials[activeTestimonial].text}"
                  </p>
                  <p className="text-[11px] font-semibold text-foreground">{testimonials[activeTestimonial].name}</p>
                ...
                {testimonials.map((_, i) => (
                  <button
                    ...
                    className={`h-0.5 rounded-full transition-all duration-300 ${
                      i === activeTestimonial ? 'w-5 bg-primary' : 'w-1.5 bg-muted-foreground/20'
                    }`}
                  />
                ))}
              </div>
              ...
              <div className="flex -space-x-2.5">
                {avatars.map((avatar, i) => (
                  <img
                    ...
                    className="w-7 h-7 rounded-full border-2 border-background object-cover shadow-sm relative"
                    style={{ zIndex: avatars.length - i }}
                  />
                ))}
                <div className="w-7 h-7 rounded-full border-2 border-background bg-primary/10 flex items-center justify-center relative" style={{ zIndex: 0 }}>
                  <span className="text-[8px] font-bold text-primary">+50K</span>
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold text-foreground">Loved by developers worldwide</p>
                <p className="text-[10px] text-muted-foreground">Join 50,000+ builders using AdiGon AI daily</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <svg viewBox="0 0 272 92" className="h-3.5 opacity-50" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-8 relative overflow-y-auto">
        ...
        <div className="w-full max-w-[400px] animate-fade-in-up">
          ...
          <div className="flex items-center justify-center gap-2 mt-4 lg:hidden">
            ...
            <svg viewBox="0 0 272 92" className="h-3 opacity-40" xmlns="http://www.w3.org/2000/svg">
              <path d="M115.75 47.18c0 12.77-9.99 22.18-22.25 22.18s-22.25-9.41-22.25-22.18C71.25 34.32 81.24 25 93.5 25s22.25 9.32 22.25 22.18zm-9.74 0c0-7.98-5.79-13.44-12.51-13.44S80.99 39.2 80.99 47.18c0 7.9 5.79 13.44 12.51 13.44s12.51-5.55 12.51-13.44z" fill="hsl(var(--muted-foreground))"/>
              <path d="M163.75 47.18c0 12.77-9.99 22.18-22.25 22.18s-22.25-9.41-22.25-22.18c0-12.85 9.99-22.18 22.25-22.18s22.25 9.32 22.25 22.18zm-9.74 0c0-7.98-5.79-13.44-12.51-13.44s-12.51 5.46-12.51 13.44c0 7.9 5.79 13.44 12.51 13.44s12.51-5.55 12.51-13.44z" fill="hsl(var(--muted-foreground))"/>
              <path d="M209.75 26.34v39.82c0 16.38-9.66 23.07-21.08 23.07-10.75 0-17.22-7.19-19.66-13.07l8.48-3.53c1.51 3.61 5.21 7.87 11.17 7.87 7.31 0 11.84-4.51 11.84-13v-3.19h-.34c-2.18 2.69-6.38 5.04-11.68 5.04-11.09 0-21.25-9.66-21.25-22.09 0-12.52 10.16-22.26 21.25-22.26 5.29 0 9.49 2.35 11.68 4.96h.34v-3.61h9.25zm-8.56 20.92c0-7.81-5.21-13.52-11.84-13.52-6.72 0-12.35 5.71-12.35 13.52 0 7.73 5.63 13.36 12.35 13.36 6.63 0 11.84-5.63 11.84-13.36z" fill="hsl(var(--muted-foreground))"/>
              <path d="M225 3v65h-9.5V3h9.5z" fill="hsl(var(--muted-foreground))"/>
